...
void receivedMsgSerial()
{
  ...
  {
    if (nodeList.n_nodes == 1 && (serverMessage[CMDm] & 0x3F) == r)
    {
      comObj.resetFunc();
    }
  ...
  else // If message is for an arduino of the network
  {
    ...
    {
      uint8_t cli_id = (serverMessage[IDm] >> 5) & 0x07;

      Serial.print("C");
      Serial.print(String(cli_id));
      Serial.println("Error! ID isn't in the network");
    }
  }
}
...
void setup()
{
  ...
  Serial.print("D Arduino network initialized correctly!\n");
...
void loop()
{
  if (millis() - ts >= 5000) // Debug
  {
    ts = millis();
    //Serial.print("C1 Waiting user input...\n");
  }
